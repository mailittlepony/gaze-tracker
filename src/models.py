...
def load_models(model_dir=None):
    global gaze_interpreter, gaze_input, gaze_output
    global face_interpreter, face_input, face_output

    if model_dir is None:
        repo_root = Path(__file__).resolve().parents[1]
        model_dir = repo_root / "models"

    gaze_model_path = os.path.join(model_dir, TFLITE_MODEL_PATH)
    face_model_path = os.path.join(model_dir, FACE_REC_MODEL_PATH)

    gaze_interpreter = Interpreter(model_path=str(gaze_model_path))
    ...
    face_interpreter = Interpreter(model_path=str(face_model_path))
